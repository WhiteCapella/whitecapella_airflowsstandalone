...
with DAG(
    ...
    task_signal = BashOperator(
            task_id="signalfile_make",
            bash_command="""
            mkdir -p ~/data/signal/{{ds_nodash}}
            touch ~/data/signal/{{ds_nodash}}/_DONE
            """
    )
    ...
    task_sort >> task_count >> task_signal >> task_end
